...
opetion_pref = st.selectbox(
    '都道府県',
    (pref_list)
)
df_pref_mean = df_pref_mean[df_pref_mean['都道府県名'] == opetion_pref]
...
opetion_year = st.selectbox(
    '集計年',
    (year_list)
)

wage_list = ['一人当たり賃金（万円）', '所定内給与額（万円）', '年間賞与その他特別給与額（万円）']
opetion_wage = st.selectbox(
    '賃金の種類',
    (wage_list)
)

df_mean_categ = df_jp_category[(df_jp_category['集計年'] ==opetion_year)]
max_x = df_mean_categ[opetion_wage].max() + 50
